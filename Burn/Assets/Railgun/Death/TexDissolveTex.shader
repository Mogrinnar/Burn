Shader "Burn/TexDisolveTex"
{
	Properties
	{
		_OverTex("Over Texture", 2D) = "green" {}
		_UnderTex("Under Texture", 2D) = "red" {}
...
#pragma fragment frag
	...
	{
		// base color
		float4 color = tex2D(_OverTex, input.texCoord);
		float4 under_color = tex2D(_UnderTex, input.texCoord);
		...
		float useUnderTex = noiseSample - threshold < 0;
		color = (1 - useUnderTex)*color + useUnderTex*under_color;
